...
export function Register() {
  ...
  const userRoles = ['Admin', 'Market Manager', 'SuperAdmin', 'District Manager', 'Employee'];
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(ntid, fullname, DoorCode, selectedRole, selectedSubRole, password);
            ...
            <div className="d-flex justify-content-center mt-5">
              <form onSubmit={handleSubmit} className="col-12 col-md-4 shadow-lg p-3 rounded">
                ...
                <input
                  ...
                  className={`form-control my-1 ${!isNtidValid ? 'input-error' : ''}`}
                  ref={NTIDRef}
                />
                <input
                  type="text"
                  ...
                  className={`form-control my-1 ${!isFullnameValid ? 'input-error' : ''}`}
                  ref={fullNameRef}
                />
                <input
                  type="text"
                ...
                <Dropdown>
                  <Dropdown.Toggle
                    id="roleDropdown"
                    className={`form-control bg-transparent text-dark text-start border-secondary shadow-none my-1 ${!isRoleValid ? 'input-error' : ''}`}
                  >
                    {selectedRole || 'Select Role'}
                  </Dropdown.Toggle>
                  <Dropdown.Menu>
                    {userRoles.map((role, index) => (
                      <Dropdown.Item key={index} onClick={() => handleSelect(role)}>
                        {role}
                      </Dropdown.Item>
                    ))}
                  </Dropdown.Menu>
                </Dropdown>
                {selectedRole && !['Employee', 'District Manager', 'SuperAdmin'].includes(selectedRole) && (
                  <Dropdown>
                    <Dropdown.Toggle id="subRoleDropdown" className="form-control bg-transparent text-dark text-start border-secondary ">
                      {selectedSubRole || 'Select Subrole'}
                    </Dropdown.Toggle>
                    <Dropdown.Menu>
                      {subUserroles.map((role, index) => (
                        <Dropdown.Item key={index} onClick={() => handleSubSelect(role)}>
                          {role}
                        </Dropdown.Item>
                      ))}
                    </Dropdown.Menu>
